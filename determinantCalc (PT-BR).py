...
for i in range(0, n):
    while len(mTemp) != n:
        mTemp = input(f"Digite os elementos da {i+1}ª linha (Utilize uma vígula para separar eles): ").strip(" ").split(",")
    for p in range(0, n):
...
print(f"O determinante da matrix é: {showAsInt(calcDet(m, n))}")
